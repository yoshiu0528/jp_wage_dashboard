...
df_pref_map['一人あたり賃金（相対値）'] = ((df_pref_map['一人当たり賃金（万円）']- df_pref_map['一人当たり賃金（万円）'].min())/(df_pref_map['一人当たり賃金（万円）'].min()))
...
layer = pdk.Layer(
    ...
    threshold=0.3,
    get_weight = '一人あたり賃金（相対値）'#複数行あったときにどの列を可視化するかの指定
)
...
st.header('■年令階級別の全国一人当たり平均賃金（万円）')

df_mean_bubble = df_jp_ind[df_jp_ind['年齢'] != '年齢計']

fig = px.scatter(df_mean_bubble,# データの挿入
                x='一人当たり賃金（万円）', # 横軸
                y='年間賞与その他特別給与額（万円）',# 縦軸
                range_x=[150, 700],# 軸の範囲
                range_y=[0, 150],
                size='所定内給与額（万円）',# バブルの設定の引数
                size_max = 38,# バブルサイズの最大値の指定
                color='年齢',# どの引数で色分けするかを指定
                animation_frame='集計年',# アニメーション表示するための設定
                animation_group='年齢')
st.plotly_chart(fig)
